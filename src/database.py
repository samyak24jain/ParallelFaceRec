# Script to create an image database compatible with the recognizers
# Allows the script to crop the face out of the images and convert them 
# to grayscale and a fixed size.
...
def resize():
    ...
    # os.rename("cropped.jpg", str_user)
    croppedImage = cv2.imread("cropped.jpg")
    gray = cv2.cvtColor(croppedImage, cv2.COLOR_BGR2GRAY)
    size = 92, 112
    im_resized = cv2.resize(gray, size)
    # if not os.path.exists("db"):
    #     os.mkdir("db")
    cv2.imwrite("cropped_resized.jpg", im_resized)
